...
export default function SearchForm() {
  ...
  const [results, setResults] = useState<{ score: number; text: string; link: string }[]>([])
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <div className="space-y-2">
        <Label className="text-sky-800">Limit: {limit}</Label>
        <Slider
          min={1}
          max={50}
          value={[limit]}
      ...
      <div className="space-y-2">
        <Label className="text-sky-800">Min Score: {minScore.toFixed(2)}</Label>
        <Slider
          min={0}
          max={1}
          step={0.01}
        ...
        {results.map((res, idx) => (
          <Card key={idx} className="border-sky-200">
            <CardContent className="p-4">
              <p className="text-sky-900 font-medium">Score: {res.score.toFixed(4)}</p>
              <p className="text-gray-800 mt-1">{res.text}</p>
              {res.link && (
                <a
                  href={res.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-sky-600 underline mt-2 block"
                >
                  View Link
                </a>
              )}
